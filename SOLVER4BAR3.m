...
x0 = [deg2rad(15);deg2rad(275.33);0;0;0;0];
...
for i=1:nSamples
    ...
    f = @(x) [  a*cos(th6) + b*cos(x(1)) - c*cos(x(2)) - d*cos(th4-delta1);...
                a*sin(th6) + b*sin(x(1)) - c*sin(x(2)) - d*sin(th4-delta1);...
               -a*om6*sin(th6) - b*x(3)*sin(x(1)) + c*x(4)*sin(x(2)) + d*om4*sin(th4-delta1);...
                a*om6*cos(th6) + b*x(3)*cos(x(1)) - c*x(4)*cos(x(2)) - d*om4*cos(th4-delta1);...
               -a*al6*sin(th6) - a*om6^2*cos(th6) - b*x(5).*sin(x(1)) - b*x(3).^2.*cos(x(1)) + c*x(6).*sin(x(2)) + c*x(4).^2.*cos(x(2)) + d*al4*sin(th4-delta1) + d*om4^2*cos(th4-delta1);...
                a*al6*cos(th6) - a*om6^2*sin(th6) + b*x(5).*cos(x(1)) - b*x(3).^2.*sin(x(1)) - c*x(6).*cos(x(2)) + c*x(4).^2.*sin(x(2)) - d*al4*cos(th4-delta1) + d*om4^2*sin(th4-delta1)];
...
%anchor points
R02 = zeros(2,length(theta22));

R04complex = d*exp(1j*(theta44-delta1));
R04 = [real(R04complex);imag(R04complex)];

RBcomplex =  d*exp(1j*(theta44-delta1)) + c*exp(1j*theta77);
RB = [real(RBcomplex);imag(RBcomplex)];

RPcomplex =  RA + p*exp(1j*(2*pi-(delta2-theta88)));
...
link1 = plot([R02(1,1),R04(1,1)],[R02(2,1),R04(2,1)], 'linewidth',2);
tt = title(sprintf('t = %3.3f',t(1)));
 
link5 = plot([RA(1,1), RP(1,1)],[RA(2,1), RP(2,1)],'linewidth',2);
% link6 = plot([RB(1,1), RP(1,1)],[RB(2,1), RP(2,1)],'linewidth',2);
...
for i=2:nSamples
    ...
    link1.XData = [R02(1,i),R04(1,i)];
    link1.YData = [R02(2,i),R04(2,i)];
    ...
    link5.XData = [RA(1,i),RP(1,i)];
    link5.YData = [RA(2,i),RP(2,i)];
%     
%     link6.XData = [RB(1,i),RP(1,i)];
%     link6.YData = [RB(2,i),RP(2,i)];
    
%     AAvec.XData = [RcomA(1,i),RcomA(1,i) + AcomA(1,i)];
%     AAvec.YData = [RcomA(2,i),RcomA(2,i) + AcomA(2,i)];
%     
%     ABvec.XData = [RcomB(1,i),RcomB(1,i) + AcomB(1,i)];
%     ABvec.YData = [RcomB(2,i),RcomB(2,i) + AcomB(2,i)];
%     
%     ACvec.XData = [RcomC(1,i),RcomC(1,i) + AcomC(1,i)];
%     ACvec.YData = [RcomC(2,i),RcomC(2,i) + AcomC(2,i)];
%     
