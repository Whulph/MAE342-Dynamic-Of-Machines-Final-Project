...
RcomAcomplex = a/2*exp(1i*theta2);
RcomA = [real(RcomAcomplex);imag(RcomAcomplex)];

VcomAcomplex = 1i*omega2.*(a/2).*exp(1i*theta2);
VcomA = [real(VcomAcomplex);imag(VcomAcomplex)];

AcomAcomplex = (-omega2.^2 + 1i*alpha2).*(a/2).*exp(1i*theta2);
AcomA = [real(AcomAcomplex);imag(AcomAcomplex)];

%link "c"
RBcomplex = -d*exp(1j*theta1) + c*exp(1i*theta4);
RB = [real(RBcomplex);imag(RBcomplex)];

% RcomCcomplex = d + (c/2)*exp(1i*theta4);
% RcomC = [real(RcomCcomplex);imag(RcomCcomplex)];
% 
% VcomCcomplex = 1i*omega4.*(c/2).*exp(1i*theta4);
% VcomC = [real(VcomCcomplex);imag(VcomCcomplex)];
% 
% AcomCcomplex = (-omega4.^2 + 1i*alpha4).*(c/2).*exp(1i*theta4);
% AcomC = [real(AcomCcomplex);imag(AcomCcomplex)];

%link "b"
% RcomBcomplex = a*exp(1i*theta2) + (b/2)*exp(1i*theta3);
% RcomB = [real(RcomBcomplex);imag(RcomBcomplex)];
% 
% VcomBcomplex = 1i*omega2.*a.*exp(1i*theta2) + 1i*omega3.*(b/2).*exp(1i*theta3);
% VcomB = [real(VcomBcomplex);imag(VcomBcomplex)];
% 
% AcomBcomplex = (-omega2.^2 + 1i*alpha2).*a.*exp(1i*theta2) + (-omega3.^2 + 1i*alpha3).*(b/2).*exp(1i*theta3);
% AcomB = [real(AcomBcomplex);imag(AcomBcomplex)];

%anchor points
R02 = zeros(2,length(theta2));
R04complex = -d*exp(1j*theta1);
R04 = [real(R04complex);imag(R04complex)];
% R04 = [d+zeros(size(theta2));zeros(size(theta2))];

% RBcomplex = RA + b*exp(1j*theta55);
% RB = [real
